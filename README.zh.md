...
- **多方言架构** — 插件式方言系统，具有明确的抽象接口。内置 Kingbase（人大金仓）完整实现和 MySQL 完整实现。
- **连接管理** — 保存、加载、测试和切换多个数据库连接，配置持久化到 `~/.claude-code-db-plugin/connections.json`，**密码使用 Fernet 加密存储**。
- **SQL 查询执行** — 编写和执行任意 SQL，支持参数化查询（防 SQL 注入）、执行计时、结果展示和自动历史记录。
- **CRUD 操作** — 完整的增删改查支持，自动解析主键和分页功能。**支持数据浏览表格内联编辑，主键可自定义，冲突实时检测**。
- **假数据生成** — 基于表结构的智能数据生成，20+ 字段名模式匹配（姓名、邮箱、电话、地址等），**内置 4407 条中国地址数据**，支持时间类型/整数模式配置、自定义规则和规则文件，支持数据预览后插入。
...
- **日志系统** — 基于 `RotatingFileHandler` 的日志记录，自动写入 `logs/app.log`，单文件最大 5MB，保留 3 个备份。
...
| 数据库驱动 | pymysql (1.0+) | MySQL 数据库连接 |
| 假数据 | Faker (20.0+) | 真实测试数据生成 |
| Excel I/O | openpyxl (3.1+) | Excel 文件读写 |
| 加密 | cryptography (3.4+) | Fernet 对称加密，用于密码存储 |
...
使用 `pymysql` 驱动完整实现，包含：
- 完整的元数据查询（`information_schema`）
- 参数化 CRUD 操作（反引号 `` ` `` 引用标识符）
- 完整的 MySQL 类型映射
- 事务处理（`autocommit = False`，成功提交，失败回滚）
- 支持 `charset=utf8mb4`
...
| **MySQL** | 已完整实现 | pymysql | 3306 | 使用 `pymysql` 驱动；基于 `information_schema` 的元数据查询；支持 `utf8mb4` |
...
1. 在数据浏览界面，点击 **编辑模式** 按钮进入可编辑状态。
2. 点击 **新增行** 添加空白行，或直接在现有单元格中修改数据。
3. **主键编辑**：新增行的主键列可编辑
   - 若主键留空，保存时自动生成唯一值
   - 若手动输入主键，实时检测冲突，冲突时拒绝并提示
4. 修改后的单元格显示黄色背景，新增行显示绿色背景。
5. 支持的操作：
   - **新增行** — 添加空白行，自动聚焦到主键列
   - **删除行** — 选中行或单元格后点击删除
   - **保存更改** — 将本地修改写入数据库（先执行删除，再更新，最后插入）
   - **撤销更改** — 恢复所有本地修改
6. 点击 **只读模式** 退出编辑状态。
...
4. 设置生成条数（1–10,000）。
5. **高级选项**：
   - **时间类型**：选择日期时间生成范围（当前时间、一周前、一个月前、一年后等 10 种模式）
   - **整数模式**：非负整数 或 允许负数
   - **地址文件**：可指定自定义地址数据文件（默认内置 4407 条中国地址）
   - **自定义规则**：支持配置列名模式到 Faker 方法的映射
   - **规则文件**：支持为指定列配置从本地文件随机取值的规则
6. 点击 **预览** 查看最多 5 条样例数据。
7. 点击 **生成并插入** 生成并插入全部数据。
8. 弹窗显示成功和失败的插入数量。
...
| 文件 | 位置 | 用途 |
|------|------|------|
| `connections.json` | `~/.claude-code-db-plugin/connections.json` | 保存的数据库连接配置（JSON 数组，密码使用 Fernet 加密） |
| `.key` | `~/.claude-code-db-plugin/.key` | Fernet 加密密钥（自动生成，请勿泄露） |
